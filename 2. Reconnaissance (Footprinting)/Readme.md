🛰️ Reconnaissance (Footprinting)

🧭 Footprinting Overview
🌍 Website Recon
🕷️ Katana – Web Crawler
🗃️ Waybackurls – Archived URLs
🛠️ Web Tech Stack Detection
🧾 WHOIS Lookup
🛰️ CDN Detection
🌐 Domain ➡️ IP ➡️ Location
📡 AS / ASN / IP Lookup
🧭 IP Traceroute
🧠 IP History
🔓 CDN Bypass Techniques
🔁 Reverse IP Domain Lookup
📧 Reverse Mail Server Lookup
🌐 Reverse NS Lookup
🔍 GHDB & Google Dorking
💥 Google Dorks – Exploit Search
🧰 GHDB Tools
🌐 Shodan Recon – GUI & CLI
🛰️ LeakIX – Shodan Alternative
🌏 FOFA Recon – Chinese Alt
🧑‍💻 GitHub Recon
🌐 Open-Source Intelligence (OSINT)
🖼️ Image Metadata
📥 IP Logger
✉️ Email Tracking Protocols
📬 Email Header Analysis
🕵️‍♂️ Email Spoofing
🧑‍🤝‍🧑 People Search & OSINT Tools
🕸️ Maltego and SpiderFoot
✅ Reconnaissance Checklist
📜 NDA & Legal Compliance in Pentesting

📖 Reference: Notes inspired by guidance from Mr. Sachin Verma (Armour Infosec) and enriched with further improvements and updates.
